...
public class Background {
	...
	public void doAndDraw(Canvas c, int height, int width) {
		...
		c.drawText((double) xScale + ":" + (double) yScale, 100, 100, p);

	}

	public void doAndDraw1(Canvas c, int height, int width) {
		float xScale = 1;
		float yScale = 1;
		Matrix matrix;
		Paint p = new Paint();
		...
		switch (adaptation) {
		case ADAPTATION_PLACE:
			c.drawBitmap(sourceImage, anchorPointScreen.getX()
					- anchorPointBitmap.getX(), anchorPointScreen.getY()
					- anchorPointBitmap.getY(), p);
			return;

		case ADAPTATION_STRETCHING:
			xScale = (float) width / (float) sourceImage.getWidth();
			yScale = (float) height / (float) sourceImage.getHeight();
			matrix = new Matrix();
			matrix.reset();
			matrix.postScale(xScale, yScale);
			c.drawBitmap(sourceImage, matrix, p);
			return;
		...
		case ADAPTATION_PADDING:
			...
			float _x1 = (float) (anchorPointScreen.getX() - anchorPointBitmap
					.getX()) / (float) sourceImage.getWidth();
			int x1 = 0;
			if (_x1 > 0)
				x1 = (int) Math.ceil(_x1);

			x = anchorPointScreen.getX() - anchorPointBitmap.getX() - x1
					* sourceImage.getWidth();

			_x1 = (float) (width - (anchorPointScreen.getX()
					- anchorPointBitmap.getX() + sourceImage.getWidth()))
					/ (float) sourceImage.getWidth();
			if (_x1 > 0)
				x1 += (int) Math.ceil(_x1);
			x1++;

			float _y1 = (float) (anchorPointScreen.getY() - anchorPointBitmap
					.getY()) / (float) sourceImage.getHeight();
			int y1 = 0;
			if (_y1 > 0)
				y1 = (int) Math.ceil(_y1);

			y = anchorPointScreen.getY() - anchorPointBitmap.getY() - y1
					* sourceImage.getHeight();

			_y1 = (float) (height - (anchorPointScreen.getY()
					- anchorPointBitmap.getY() + sourceImage.getHeight()))
			...
			for (int i = 0; i < x1; i++) {
				for (int j = 0; j < y1; j++) {
					c.drawBitmap(sourceImage, x + i * sourceImage.getWidth(), y
							+ j * sourceImage.getHeight(), p);
				}
			}

			return;

		case ADAPTATION_SMART:
			break;
