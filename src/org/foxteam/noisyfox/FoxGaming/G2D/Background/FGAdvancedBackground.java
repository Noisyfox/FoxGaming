 ...
 * 
 */
public class FGAdvancedBackground implements FGBackground {
			...
			for (int i = 0; i < x1; i++) {
				for (int j = 0; j < y1; j++) {
					verticleBuffer = sourceImage.getFloatBuffer(new float[] {
							...
							left + xt + (i + 1) * imageWidth + 1,
							top + yt + (j + 1) * imageHeight + 1 });
					sourceImage.getTargetGl().glVertexPointer(2, GL10.GL_FLOAT,
							0, verticleBuffer);
					sourceImage.getTargetGl().glDrawArrays(
