 ...
 * to a Sprite to further vary its appearance. As with all Layer subclasses, a
 * Sprite's location can be changed and it can also be made visible or
 * invisible.
 * <P>
 * <h3>Sprite Frames</h3>
 * The raw frames used to render a Sprite are provided in a single Image object,
 * which may be mutable or immutable. If more than one frame is used, the Image
 * is broken up into a series of equally-sized frames of a specified width and
 * height. As shown in the figure below, the same set of frames may be stored in
 * several different arrangements depending on what is the most convenient for
 * the game developer. <br>
 * <center><img src="doc-files/frames.gif" width=777 height=402
 * ALT="Sprite Frames"></center> <br>
 * <p>
 ...
 * The default frame sequence mirrors the list of available frames, so there is
 * a direct mapping between the sequence index and the corresponding frame
 * index. This also means that the length of the default frame sequence is equal
 * to the number of raw frames. For example, if a Sprite has 4 frames, its
 * default frame sequence is {0, 1, 2, 3}. <center><img
 * src="doc-files/defaultSequence.gif" width=182 height=269
 * ALT="Default Frame Sequence"></center>
 * <P>
 * The developer must manually switch the current frame in the frame sequence.
 * This may be accomplished by calling {@link #setFrame}, {@link #prevFrame()},
 * or {@link #nextFrame()}. Note that these methods always operate on the
 * sequence index, they do not operate on frame indices; however, if the default
 * frame sequence is used, then the sequence indices and the frame indices are
 * interchangeable.
 * <P>
 * If desired, an arbitrary frame sequence may be defined for a Sprite. The
 * frame sequence must contain at least one element, and each element must
 * reference a valid frame index. By defining a new frame sequence, the
 * developer can conveniently display the Sprite's frames in any order desired;
 * frames may be repeated, omitted, shown in reverse order, etc.
 * <P>
 ...
 * mosquito flaps its wings three times and then pauses for a moment before the
 * cycle is repeated. <center><img src="doc-files/specialSequence.gif" width=346
 * height=510 ALT="Special Frame Sequence"></center> By calling
 * {@link #nextFrame} each time the display is updated, the resulting animation
 * would like this: <br>
 ...
 * in terms of an arbitrary pixel within its frame, especially if transforms are
 * applied to the Sprite.
 * <P>
 * Therefore, Sprite includes the concept of a <em>reference pixel</em>. The
 * reference pixel is defined by specifying its location in the Sprite's
 * untransformed frame using {@link #defineReferencePixel
 * defineReferencePixel(x,y)}. By default, the reference pixel is defined to be
 * the pixel at (0,0) in the frame. If desired, the reference pixel may be
 * defined outside of the frame's bounds.
 * <p>
 * In this example, the reference pixel is defined to be the pixel that the
 * monkey appears to be hanging from:
 * <p>
 ...
 * coordinate system. The developer can also use {@link #setRefPixelPosition
 * setRefPixelPosition(x,y)} to position the Sprite so that reference pixel
 * appears at a specific location in the painter's coordinate system. These
 * methods automatically account for any transforms applied to the Sprite.
 * <p>
 ...
 * include rotations in multiples of 90 degrees, and mirrored (about the
 * vertical axis) versions of each of the rotations. A Sprite's transform is set
 * by calling {@link #setTransform setTransform(transform)}.
 * <p>
 * <center><img src="doc-files/transforms.gif" width=355 height=575
 * ALT="Transforms"></center> <br>
 * When a transform is applied, the Sprite is automatically repositioned such
 * that the reference pixel appears stationary in the painter's coordinate
 * system. Thus, the reference pixel effectively becomes the center of the
 * transform operation. Since the reference pixel does not move, the values
 * returned by {@link #getRefPixelX()} and {@link #getRefPixelY()} remain the
 * same; however, the values returned by {@link #getX getX()} and {@link #getY
 * getY()} may change to reflect the movement of the Sprite's upper-left corner.
 * <p>
 * Referring to the monkey example once again, the position of the reference
 * pixel remains at (48, 22) when a 90 degree rotation is applied, thereby
 * making it appear as if the monkey is swinging from the branch:
 * <p>
 ...
 * visibility). Erasing the Sprite is always the responsibility of code outside
 * the Sprite class.
 * <p>
 * <p>
 * Sprites can be implemented using whatever techniques a manufacturers wishes
 * to use (e.g hardware acceleration may be used for all Sprites, for certain
 * sizes of Sprites, or not at all).
 * <p>
...
public class Sprite extends Layer {
	 ...
	 * 
	 */
	protected final void paint(Canvas c) {
